...
# Minimal requirements (expected in requirements.txt):
# flask, gunicorn, requests, numpy, pandas, scikit-learn, matplotlib, seaborn, shap, lifelines, loguru

import os, time, threading, traceback, uuid, io
...
TRAIN_INTERVAL = 60             # seconds between training attempts
WINDOW_SIZE = 30                # sequence length
MAX_HORIZON_SECONDS = 15 * 60   # 15 minutes horizon
BUFFER_MAX = 60000
...
class ExperienceBuffer:
    def __init__(self, maxlen=BUFFER_MAX):
        self.rounds = deque(maxlen=maxlen)  # {"id","crash","ts"}
        self.ids = set()
    ...
    def append_raw(self, r):
        ...
        if cp is None:
            return
        if rid is not None and rid in self.ids:
            return
        if ts is None:
            ts = int(time.time())
...
def seq_to_features(seq):
    ...
    mn = float(np.min(arr)) if len(arr) else 0.0
    mx = float(np.max(arr)) if len(arr) else 0.0
    med = float(np.median(arr)) if len(arr) else 0.0
    prop_low = float(np.sum(arr < 2.0)/len(arr)) if len(arr) else 0.0
...
class EnsembleStack:
    def __init__(self):
        ...
        self.gb = GradientBoostingClassifier(n_estimators=80)
        self.lr = LogisticRegression(max_iter=400)
        self.meta = LogisticRegression(max_iter=400)
...
# ---------------- RL fallback (evolutionary) ----------------
class EvolutionaryTrainer:
    def __init__(self, pop=20, gens=20):
        self.pop = pop
        self.gens = gens
        self.best = None
    def random_strategy(self):
        return {"thr2": np.random.uniform(0.4, 0.9),
...
evo_trainer = EvolutionaryTrainer()
...
# ---------------- PREDICTOR (logger, pending monitor, cooldowns) ----------------
class Predictor:
    def __init__(self):
                ...
                for k in keys:
                    with self.pending_lock:
                        ...
                        if not e or e["status"] != "pending":
                            continue
                        found = False; observed_ts = None
                        for c, ts in zip(crashes, timestamps):
                            if ts < e["requested_at"]:
                                continue
                            if ts > e["deadline"]:
                                continue
    ...
    def predict(self):
        ...
        if self.drift.check_and_maybe_freeze(crashes):
            return {"error": "Drift detected — Frozen."}
        ...
        for t in TARGETS:
            ...
            results.append({
                ...
                "pred_seconds": int(sec),
                "pred_time_iso": datetime.fromtimestamp(pred_ts).strftime("%H:%M:%S"),
                "combined_score": round(float(combined), 3),
                "recommended": recommended,
                "status": ("pending" if recommended else "not_recommended"),
        ...
        for r in results:
            ...
            r["suggested_stake_fraction"] = r["kelly_frac"] * 0.5
...
def fetch_history_once():
    try:
        ...
        r.raise_for_status()
        data = r.json()
        return data.get("history", data.get("rounds", []))
    except Exception:
        return []
...
def backtest_strategy(strategy_fn, df, initial_bank=1000):
    bank = initial_bank; trades=[]
    crashes = df["crash"].values; n = len(crashes)
    for i in range(WINDOW_SIZE, n-1):
        ...
        for j in range(i, n):
            if crashes[j] >= target:
                ...
                bank += payout; trades.append({"index": i, "target": target, "stake": stake, "result": "win", "payout": payout})
                found = True; break
            if j == n-1 and not found:
                bank -= stake; trades.append({"index": i, "target": target, "stake": stake, "result": "loss", "payout": -stake})
        if bank <= 0:
            break
    wins = sum(1 for t in trades if t["result"] == "win")
    losses = sum(1 for t in trades if t["result"] == "loss")
    return {"final_bank": bank, "pnl": bank - initial_bank, "wins": wins, "losses": losses, "trades": trades}
...
async function ask(){
  ...
  const main = j.main;
  document.getElementById('main').innerHTML = '<div class="big">'+main.target+'x</div><div class="muted">Prévu: '+main.pred_time_iso+' — score '+(main.combined_score*100).toFixed(1)+'%</div>';
...
def recent_route():
    ...
    with predictor.pending_lock:
        for p in list(predictor.recent)[:80]:
            pid = p.get("id")
...
if __name__ == "__main__":
    logger.info("Starting Ultra Bot")
